...
class DangleControl:
	...
	def run(self):
		...
		motorL = SwitchingControlMediator( [motorsStop, 								 # Choice 0 = Stopped \
											 [ 											 # Choice 1 = Controlled
												 Scaler([joystickForward, Scaler(joystickLeftRight, scaling = -1.0)],scaling =0.8), # Joystick  \
												 ValueIntegrator(self.sensors.upDownButton(13, 14), scaling = -0.01),	# Buttons \
												 #ValueIntegrator([OneShotButtonValue(self.sensors.button(13), triggeredValue = -0.1), OneShotButtonValue(self.sensors.button(14), triggeredValue = 0.1)]), 		 # Buttons \
												 Scaler(headingErrorValueL,scaling=0.3)]],					 				# Heading  \
											self.controls.motor(2), \
											motorEnable )
		self.highPriorityProcesses.append(motorL)
		motorR = SwitchingControlMediator( [motorsStop, 							# Choice 0 = Stopped \
											 Scaler(								# Choice 1 = Controlled
												[Scaler([joystickForward, joystickLeftRight],scaling =0.8),  			# Joystick \
													ValueIntegrator(self.sensors.upDownButton(13, 14), scaling = -0.01),	# Buttons \
													#ValueIntegrator([OneShotButtonValue(self.sensors.button(13), triggeredValue = -0.1), OneShotButtonValue(self.sensors.button(14), triggeredValue = 0.1)), 		 # Buttons  \
													Scaler(headingErrorValueL,scaling=-0.3)],						# Heading  \
											  scaling=-1.0)],
		...
		rotateLeft = OneShotButtonValue(self.sensors.button(15), triggeredValue = -22.5)
		rotateRight = OneShotButtonValue(self.sensors.button(16), triggeredValue = -22.5)
		
		#SimplePIDControlMediator(pid, PitchValue(), control, setCurrentPlusError),	# Keep level controlled \
		...
		while not done:
			...
			if running:
				...
				lineAngle = imageProcessor.captureAndDisplay()
